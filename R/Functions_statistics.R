...
U_test_vectors <- function(vec1, vec2, paired = F) {
  ...
  if (all(unique(vec1 == 0)) & all(unique(vec2 == 0)))
    return(1)
  if(identical(vec1,vec2)) return(1)
...
U_test_tbl <- function(paired, ...) {
  ...
  error = function(e) { return("Sample size needs to be identical for a paired test!")})
  }
  return(p_values)
}
  ...
  p_value_list <-
    map_depth(columns, 2, function(cols, sum,paired)
      map2(sum, cols, function(s, x, s_o, x_o,paired)
        U_test_vectors(unlist(s_o[x_o]), unlist(s[x]),paired), sum[[1]], cols[[1]],paired), summary,paired)
    ...
    p_list <-
      map2(column_vector, summary_list, function(d1_map, sum_map, d1_orig, sum_orig)
        map2(d1_orig, sum_orig, function(d1_orig_map,
                                         ...
                                         sum_map,
                                         d1_map)
          U_test_vectors(unlist(sum_map[d1_map]), unlist(sum_orig_map[d1_orig_map])), sum_map, d1_map), column_vector, summary_list)
