  ...
  geom <-
    geom_ribbon(data = plot_data,
              aes(x = x_axis,
                  ymin = 1,
                  ymax = IV,
                  color = measurement, fill = measurement), size = size)
  return(geom)
}
...
#' @examples
  create_matplot_plots <- function(IVs, path, series,measurement = "", filter, plot_columns, type,line_size,base_size,axis_size) {
    ...
    for (plot_column in plot_columns) {
      ...
      if (length(series) == 1) {
        if (length(filter) > 1) { for (i in 2:length(filter)) {
        ...
        else{plot_list <- c(plot_list, list(add_to_matplot(tail(plot_list,1)[[1]], IVs[[series[1]]],plot_column, filter[1],paste0(series[1],measurement[1]), line_size = line_size)))
        }
      }
      else{ for (i in 2:length(series)) {
  ...
  create_median_plots <- function(IVs, path, series, measurement = "", filter, plot_columns, type,line_size = 1.3, base_size, axis_size, sdMAD) {
    ...
    for (plot_column in plot_columns) {
      ...
      if (length(series) == 1) {
        ...
        else{plot_list <- c(plot_list, list(add_to_median_plot(tail(plot_list,1)[[1]], IVs[[series[1]]],plot_column, filter[1],factor(paste0(series[1],measurement[1])), line_size = line_size,sdMAD)))
        }
      }
      else{ for (i in 2:length(series)) {
